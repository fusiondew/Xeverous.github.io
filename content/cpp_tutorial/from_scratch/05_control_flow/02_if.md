...
If you want only the negative branch, just flip the condition: `if (!condition)`. or reverse the operator (eg `<` instead of `>=`).
...
int main()
{
    ...
    {
        std::cout << x << " is even\n";
        // other statements can follow here
    }
    else
    {
        std::cout << x << " is odd\n";
...
if (condition)
{
    ...
    {
        if (some_other_condition) // here braces can be ommited
            something_diferent;
    }
}
else
{
    if (very_different_condition) // // here braces can be ommited
...
It compiles because assignment evaluates to `x` converting it's value to type `bool` which in this case will always be the same.
...
Some coding styles recommend to swap the order, is such case `0 == x` has no difference but `0 = x` would not compile. This impacts readability though.
...
The program above does not print, but there are no errors. 
...
Solution: declare `y` before if-else block.
...
The problem is that the syntax is unclear and since the operator does not require any full statement it can be abused. Especially when nested. Additionally, the operator itself has [complex rules](https://en.cppreference.com/w/cpp/language/operator_other#Conditional_operator) regarding evaluation.

It's recommended not to use this operator at all. It creates shorter code but it's much harder to read and has much more places for potential bugs.
...
- Test whether your compiler emits a warning about assignment in condition

TODO test code and formatting

<details><summary> Calculator solution </summary>
