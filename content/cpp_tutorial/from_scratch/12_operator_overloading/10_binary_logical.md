...
A true mastery of extraordinary operator overloads is achieved in Boost Spirit. [This short article](https://en.wikipedia.org/wiki/Spirit_Parser_Framework) gives an overview of the library purpose - templated overloaded operators are used to form grammar specification allowing to parse and lex any text. This sort of solution is known as DSL (domain specific language) where a different language happens to be inside another - here unique grammar specification language using classes (which behave as commands/keywords) and overloaded operators.
